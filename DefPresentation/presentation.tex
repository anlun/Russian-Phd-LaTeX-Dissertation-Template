...
\fr{\large Структура д-ва корректности компиляции из обещающей модели в аксиоматическую модель \ARMax}{
  \iteN{
    ...
    { \footnotesize
    \[\forall P, G. \; G \in \semState{P}{\ARMax} \Rightarrow G \vdash \tup{\emptyset, \emptyset} \to^{*} G \vdash \tup{G.E, G.W}.\]
    }
    \item Показывается, что обещающая модель симулирует обход \ARMax-исполнения. \\
    \vspace{.3cm}
    { \footnotesize
    \textbf{Теорема 1.} $S_{G}(\tup{\emptyset, \emptyset}, {\sf init}_{\Promise}) $. \\
