...
\fr{\LARGE Структура д-ва корректности компиляции из обещающей модели в модель \ARMpop}{
  \Large
  \iteN{
    \item Вводится инструментированная ограниченная версия модели $\ARMpop$, т.н. $\ARMt$.
    \vfill
    \item Доказывается эквивалентность между моделями $\ARMt$ и $\ARMpop$.
    \vfill
    \item Показывается, что обещающая модель симулирует модель $\ARMt$.
  }
}
...
\fr{\LARGE Структура д-ва корректности компиляции из обещающей модели в аксиоматическую модель \ARMax}{
  \Large
  \iteN{
    \item Вводится операционная семантика обхода \ARMax-исполнения.
    \vfill
