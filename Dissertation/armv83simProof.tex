\chapter{Доказательство леммы о шаге симуляции обещающей модели и обхода сценария ARMv8.3}
...
\begin{proof}
  ...
  Мы знаем, что $\tup{w', w} \in \lCO$. Существует два варианта: $\tup{w', w} \in \lCOE$ или $\tup{w', w} \in \lCOI$.
  ...
  Рассмотрим предыдущее ребро цикла, которое мы обозначим $r$, и покажем, что во всех случаях цикл можно укоротить.  
  \begin{itemize}
    \item Если $r \in \lCOI$, то цикл может быть укорочен, т.к. $\lCOI; \lCOI \suq \lCOI$.
    \item Если $r \in \lOBS = \lRFE \cup \lFRE \cup \lCOE$, то нужно рассмотреть три варианта.
      Поскольку $\lRFE; \lCOI = \emptyset$, то $r$ не может быть из отношения $\lRFE$.
      В двух других случаях цикл может быть укорочен, т.к. $\lFRE; \lCOI \suq \lFRE$, и $\lCOE; \lCOI \suq \lCOE$.
    \item Если $r \in \lBOB$, то цикл может быть укорочен, поскольку $\lPO; \lCOI \suq \lPO$.
    \item Если $r \in \lDOB$, то цикл может быть укорочен:
      \begin{itemize}
        \item $\lADDR; \lPO^{?}; \lCOI \suq \lDOB$,
        \item $\lDATA; \lCOI \suq \lDOB$,
        \item $(\lADDR \cup \lDATA);\lRFI; \lCOI = \emptyset$,
        \item $(\lCTRL \cup \lDATA); [\lW]; \lCOI^{?}; \lCOI \suq \lDOB$.
      \end{itemize}
    \qedhere
  \end{itemize}
\end{proof}
