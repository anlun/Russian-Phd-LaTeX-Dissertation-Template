...
\subsection{Доказательство лемм и теоремы о сертификации}
...
${} \quad \exists \delta', \tT'. \Cfprom(\tId) \vdash \tT \promTStep^{*} \tT' \land \invCert(n - 1, \delta', k, \tId, \aT, \tT')$.
\begin{proof}
  Зафиксируем $n, \tId, \aT, \tT$. Введём следующие обозначения:
  \[\begin{array}{l c l}
    ...
    \item $\tape(\cpath) = \tapeWrite{\Wstate}$, где состояние $\Wstate$ указывает, что экземпляр не завершён.
      ...
      \[\begin{array}{l l}
        ...
        5. & \forall \cpath_{read} < \cpath_{\LD} < \cpath_{\delta-read}, \w, \R. \cpath < \cpath_{read} \land {} \\
           ...
           & \qquad \R(\loc) \le \tau. \\
      \end{array}\]
      Поскольку метки времени являются элементами плотного множества $\mathbb{Q}$, то в силу 
...
\begin{proof}%[Доказательство теоремы \ref{cert-thm}]
  Зафиксируем $\tId, \tape \triangleq \aT.\tapef(\tId)$. Введём следующие обозначения:
  \[\begin{array}{l l l}
  ...
  Таким образом нам нужно проверить следующее утверждение:
  \[\begin{array}{l}
...
\quad \lnot \Cfprom(\tId)[\lastInstr{\cpath'}] = ``\fenceInst{\SY}") \land {} \\
 ...
 (\delta, \tId, \aT) \in \invDeltaDefThree \cap \invDeltaDefFour. \\
  ...
  Четвертый конъюнкт выполняется в силу ограничений перехода \transenv{Завершение барьера} машины ARMv8 POP.
  Пятый конъюнкт выполняется по ограничениям перехода \transenv{Завершение записи} машины ARMv8 POP и
  истинности утверждения $\invComWrite(\aT, \p)$.
