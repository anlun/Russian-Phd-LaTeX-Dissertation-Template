...
\begin{array}{@{}l}
  ...
  \quad \kw{delete} \; r2 \\
  \\
  \funcSt{sync}(cw, cr1, cr2) \triangleq \\
  \quad \readInstParam{\rlx}{rcw}{cw}; \\
  \quad \assignInst{rcwn}{rcw + 2}; \\
  \quad \writeInstParam{\rel}{cw}{rcwn}; \\
  \quad \graybox{\funcSt{syncWithReader}(rcwn, cr1);} \\
  \quad \graybox{\funcSt{syncWithReader}(rcwn, cr2)} \\
\end{array}
...
\begin{array}{l}
  ...
  \quad \kw{end} \\
  \\
  \funcSt{syncWithReader}(rcwn, cr) \triangleq \\
  \quad \repeatInst{\readInstParam{\acq}{r0}{cr}; r0 >= rcwn} \\
  \\
  \begin{array}{@{}l @{~~~~}|@{~~~~} l@{}}
    \begin{array}{@{}l@{}}
  \funcSt{rcuOnline}(cw, cr) \triangleq \\
  \quad \readInstParam{\acq}{r0}{cw}; \\
  \quad \writeInstParam{\rlx}{cr}{r0 + 1} \\
    \end{array}
  &
    \begin{array}{@{}l@{}}
  \funcSt{rcuOffline}(cw, cr) \triangleq \\
  \quad \readInstParam{\rlx}{r0}{cw}; \\
  \quad \writeInstParam{\rel}{cr}{r0}
    \end{array}
  \end{array}
\end{array}
\end{tabular}
