...
\noindent
  \emph{\textbf{Теорема \ref{armvpop:mainthm}.}
  Для любой программы $\Cfarm$ и её сценария поведения в модели ARMv8 POP,
  $\Cfarm \vdash \sinit \armStepP^{*} \s$,
  где $\s$ --- финальное состояние, $\finalStateP(\s, \Cfarm)$,
  существует сценарий поведения этой программы в обещающей модели,
...
\begin{proof}
  Let's fix the program $\Cfprom$. In the remainder of the section we write $``\s \armStepP \s'"$ instead of
  $``\Cfprom \vdash \s \armStepP \s'"$ for all machines.
We apply the result of \cref{sec:sim-armt-arm}, and change the proof goal to the simulation for the ARM+$\tau$ machine:\\
\[\begin{array}{l}
  \forall \Cfprom, %\Cfarm = compilation(\Cfprom), \\
  \aT. \; \ainit \armStep^{*} \aT \land \finalStateA(\aT, \Cfarm) \Rightarrow \\
  \quad \exists \p. \; \pinit \promStep^{*} \p \land \finalStateProm(\p, \Cfprom) \land \sameMemory(\aT, \p). \\
\end{array}\]
To prove it, we introduce a number of relations between ARM+$\tau$ and Promise states, which are
parts of the simulation relation.
...
The $\invPrefix$ relation states that every instruction instance, which has been executed by the Promise machine,
has been executed by the ARM+$\tau$ machine:
\[
\invPrefix(\aT, \p) \triangleq \forall \tId, \cpath' < \p.\TSfprom(\tId).\cpath. \;
     \aT.\tapef(\tId, \cpath') \; \text{is committed}. \\
\]
The next relations connect the memories of the machines. $\invMemOne$ states that for every write, which is committed by the
ARM+$\tau$ machine, there is a message in the Promise memory to the same location with the same value and timestamp,
and its view is less or equal to the corresponding view of the ARM request. If the $\cpath$ of the committed write
is less than the corresponding thread's pointer to the next instruction ($\p.\TSfprom(\tId).\cpath$), then the write is
...
\end{array}\]
$\invMemTwo$ connects the memories in other direction: for every message in the Promise memory (except for initial ones) there is a
committed write instruction instance in the ARM+$\tau$ machine:
\[\begin{array}{l}
...
\end{array}\]
$\invMemThree$ relates initial writes to locations: 
\[\begin{array}{l}
...
\end{array}\]
$\invView$ says that views of a Promise thread are restricted by the composition of singleton
views of writes and reads committed by the ARM thread.
For the acquire view, it counts all the writes and reads up to $\cpath$.
For the current view, it counts all the writes up to $\cpath$ and reads up to the latest
committed $\LD$ fence ($\lastLD(\tape, \cpath)$).
For the release view, it counts all writes up to the latest committed $\SY$ fence
($\lastSY(\tape, \cpath)$) and reads up to the latest committed $\LD$ fence before
the $\SY$ fence ($\lastLDSY(\tape, \cpath)$).
\[\begin{array}{l}
...
\end{array}\]
$\invState$ declares that a variable state of a Promise thread is the same as the committed state function up to
the corresponding $\cpath$ of the ARM thread:
\[\begin{array}{l}
...
\end{array}\]
$\invComWrite$ says that if an ARM thread committed a write, then all $\cpath$-previous $\SY$ fences are executed by
the corresponding Promise thread:
\[\begin{array}{l}
...
\end{array}\]
where $\lastCommittedWrite(\tape)$ is a $\cpath$ of a last write committed by the thread.
The relation is necessary for certification of the Promise machine steps.

\noindent
$\invReach$ asserts that states are reachable:%
\[\invReach(\aT, \p) \triangleq \ainit \armStepP^{*} \aT \land \pinit \promStep^{*} \p.\]

The relation $\simrelBase$ combines the aforementioned relations:
...
In the simulation, either the Promise machine is waiting for the next step of the ARM+$\tau$ machine,
or there is a Promise thread which should make at least one non-\transenv{Promise write} step
(corresponding to an instruction which the thread's $\cpath$ component is pointing to).
A Promise thread $\tId$ is waiting for the corresponding ARM thread, if
the next command to be executed, which is pointed by $\cpath$, is not fetched or committed
in the ARM thread.
\[\begin{array}{l}
...
\]
If the states are related by $\simrelPre$, there is a thread of the Promise machine which may 
take a step (which is not \transenv{Promise write}) by executing the next instruction its $\cpath$ is pointing to.
After it either the thread has to make another step ($\simrelPre(\aT, \p')$), or all threads of the
Promise machine are waiting ($\simrel(\aT, \p')$):
\begin{lemma}
\label{lem-snd}
...
\end{lemma}
\noindent
As at every specific state of the ARM+$\tau$ machine it has committed a finite number of instruction instances,
we show that the Promise machine can make a finite number of transitions to get its state to satisfy $\simrel$:
\begin{lemma}
\label{lem-trd}
$\forall (\aT, \p) \in \simrelPre. \;
...
\end{lemma}
\noindent
Suppose, the ARM+$\tau$ and Promise machine states are related by $\simrel$.
Then, we show that the ARM+$\tau$ machine may make a step. If the step is \transenv{Write commit},
then the Promise machine has to promise the corresponding message, and the states of the machines
are related by $\simrelPre \cup \simrel$. If the step is not \transenv{Write commit}, then
the Promise machine does not make a step, and the states are related by the same relation
$\simrelPre \cup \simrel$.
...
\footnote{Proofs of the lemmas can be found
  \ifext{in \cref{appendix-proofs}}{ in the extended version of the paper \cite{Podkopaev-al:ECOOP17TR}}.}

The theorem is proved by induction on the ARM+$\tau$ execution using Lemma \ref{lem-fth}.
The machine memories are the same at the end due to $\invMemOne$ and $\invMemTwo$.
The only thing, which we need to show, is that $\finalStateProm(\p, \Cfprom)$ holds.

The Promise machine cannot make a further step (each thread's $\cpath$ points out of the thread's
instruction list), as otherwise the ARM+$\tau$ machine would be able to fetch a new instruction
instance, and $\finalStateA(\aT, \Cfarm)$ would not hold. Each thread has fulfilled its promises
according to $\invMemOne$ and $\invMemTwo$.
\end{proof}
\section{Выводы}
