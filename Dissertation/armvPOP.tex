...
Когда поток $T$ становится осведомлённым о запросе $\e$, т.е.,
в терминах Flowing, $\e$ попадает в видимый для $T$ буфер,
или, в терминах POP, $\e$ становится элементом $\Prop(T)$, то
подсистема POP добавляет по ребру $(\e', \e)$ во множество $\Ord$
за каждый запрос $\e'$ из множества $\Prop(T)$, который не может быть
переупорядочен с $\e$, т.е. $\notReorderableRel{\e'}{\e}$.
...
с инициализирующеми записями в локации.
