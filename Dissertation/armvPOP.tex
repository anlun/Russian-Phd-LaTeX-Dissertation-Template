...
Каждый экземпляр идентифицируется парой $(\tId,\cpath)$, где $\tId$ --- это идентификатор потока,
а $\cpath$ --- это путь в потоке управления до экземпляра. П
...
Состояние экземпляра представляется с помощью \emph{ячейки плёнки} $\tapeCell$ (см. рис. \ref{fig:tape-arm}),
чей синтаксис симметричен синтаксису инструкций.
...
(iii) $\tapeSatisfied{\satisfiedState}{\stRequestWrite{\tId}{ \cpath}{ \loc}{\stval}}$,
      экземпляр запрос получил ответ от 
